...
namespace CBD
{
    public class DBManager
    {
        private static string ConnectionString
        {
            get
            {
                //Data Source=LAPTOP-5N3BRTAC;Initial Catalog=Lambda;Persist Security Info=True;User ID=sa;Password=***********
                //return ConfigurationManager.ConnectionStrings["TravelAndLiveConnectionString"].ConnectionString;
                return
                    "Data Source=LAPTOP-5N3BRTAC;Initial Catalog=Lambda;Persist Security Info=True;User ID=sa;Password=verdefiel3";
            }
        }
        private static string TraccarConnectionString
        {
            get
            {
                //return ConfigurationManager.ConnectionStrings["TravelAndLiveConnectionString"].ConnectionString;
                return
                    "Data Source=altaircloud.mx,1991;Initial Catalog=TL_Traccar;Persist Security Info=True;User ID=travelRoot;Password=nuevo123";
            }
        }
        ...
        public List<ConexionBD.DbParameter> OutParameters { get; private set; }
        ...
        private void OpenTraccar()
        {
            try
            {
                Connection = new SqlConnection(TraccarConnectionString);
        ...
        private object ExecuteProcedure(string procedureName, ExecuteType executeType, List<ConexionBD.DbParameter> parameters)
        {
            ...
            if (Connection != null)
            {
                if (Connection.State == ConnectionState.Open)
                {
                    ...
                    if (parameters != null)
                    {
                        ...
                        foreach (ConexionBD.DbParameter dbParameter in parameters)
                        {
        ...
        private void UpdateOutParameters()
        {
            ...
            {
                OutParameters = new List<ConexionBD.DbParameter>();
                OutParameters.Clear();
                ...
                for (int i = 0; i < Command.Parameters.Count; i++)
                {
                    ...
                    {
                        OutParameters.Add(new ConexionBD.DbParameter(Command.Parameters[i].ParameterName,
        ...
        public List<T> ExecuteDataTable<T>(string procedureName, List<ConexionBD.DbParameter> parameters) where T : new()
        {
            Open();
        ...
        public T ExecuteSingle<T>(string procedureName, List<ConexionBD.DbParameter> parameters) where T : new()
        {
            Open();
        ...
        public List<T> ExecuteList<T>(string procedureName, List<ConexionBD.DbParameter> parameters) where T : new()
        {
        ...
        public int ExecuteNonQuery(string procedureName, List<ConexionBD.DbParameter> parameters)
        {
            int returnValue;
        ...
        public object ExecuteScalar(string procedureName, List<ConexionBD.DbParameter> parameters)
        {
            object returnValue;
        ...
        public List<T> ExecuteTraccarDataTable<T>(string procedureName, List<ConexionBD.DbParameter> parameters) where T : new()
        {
            OpenTraccar();
            SqlDataAdapter dataAdapter = (SqlDataAdapter)ExecuteProcedure(procedureName, ExecuteType.DataTable, parameters);
            DataTable datatable = new DataTable();
        ...
        public T ExecuteTraccarSingle<T>(string procedureName, List<ConexionBD.DbParameter> parameters) where T : new()
        {
            OpenTraccar();
            IDataReader reader = (IDataReader)ExecuteProcedure(procedureName, ExecuteType.ExecuteReader, parameters);
            T tempObject = new T();
